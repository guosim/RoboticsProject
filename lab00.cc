...
void starFinder(PlayerClient &robot, Position2dProxy &pp, LaserProxy &lp, int row, int col, vector< vector<int> > &GRID) {
	...
	if (trigger != -1 )
	{
		...
		if((GRID[starx][stary] != 1) && (GRID[starx][stary] >= 6))
			...
			GRID[starx][stary]=6;
		/*if ((starx < row-2) && (starx > 2) && (stary < col-2) && (stary > 2))
			{
			if((GRID[starx][stary] != 1)  && (GRID[starx][stary] >= 6))
				GRID[starx][stary]+=1;
			else if ((GRID[starx][stary] != 1) && (GRID[starx][stary] < 6))
			...
			cout <<"NAH THIS IS A BLOODY WALL M8" <<endl;
...
void MoveRLeft(PlayerClient &robot,Position2dProxy &pp,double dist, double Yaw) {
	double x,y,w;
	robot.Read();
	x = pp.GetXPos();
	y = pp.GetYPos();
	...
	Yaw = Yaw + 1.57;
	double newx = x + dist*cos(Yaw);
	double newy = y + dist*sin(Yaw);

	
	cout << "MOVING RELATIVE LEFT ******************************************" << endl;

	while( ((x-newx)*(x-newx) > THRESH) && ((y-newy)*(y-newy) > THRESH))
		{
		pp.GoTo(newx,newy,Yaw);
		robot.Read();
		x = pp.GetXPos();
		y = pp.GetYPos();
		w = pp.GetYaw();
...
void MoveRRight(PlayerClient &robot,Position2dProxy &pp,double dist, double Yaw) {
	double x,y,w;
	robot.Read();
	x = pp.GetXPos();
	y = pp.GetYPos();
	...
	cout << "MOVING RELATIVE RIGHT ******************************************" << endl;
...
void MoveForward(PlayerClient &robot,Position2dProxy &pp,double dist, double Yaw) {
	double x,y,w;
	robot.Read();
	x = pp.GetXPos();
	y = pp.GetYPos();
	...
	cout << "MOVING RELATIVE FORWARD ******************************************" << endl;

	while((x-newx)*(x-newx) > THRESH)
		{
		pp.GoTo(newx,y,Yaw);MoveRLeft(robot,pp,1,Yaw);
		robot.Read();
		x = pp.GetXPos();
		y = pp.GetYPos();
		w = pp.GetYaw();
...
void MoveRight(PlayerClient &robot,Position2dProxy &pp,double dist) {
	...
	while((x-newx)*(x-newx) > THRESH)
		...
		robot.Read();
		x = pp.GetXPos();		pp.SetSpeed(0,0,dtor(90));
		y = pp.GetYPos();
		w = pp.GetYaw();
		//cout<<"RIGHT: "<<"x = "<<x<<"	y = "<<y<<"	w = "<<w<<"\n";
		}
...
/*
	if ((lp[45] > 1) && (lp[87] < 1) && (Yaw >0.75) && (Yaw < 1.90)) //facing up, no left. go straight.
	{
		MoveUp(robot,pp,0.5);
	}

	else if ((lp[45] > 1) && (lp[87] > 1) && (Yaw >0.75) && (Yaw < 1.90) ) //facing up, turn left room
	{
		MoveUp(robot,pp,0.5);
		Turn(robot,pp,LEFTY);
		MoveLeft(robot,pp,1);
	...
	else if ((lp[45] < 1) && (lp[87] > 1) && (Yaw >0.75) && (Yaw < 1.90) ) //facing up, turn left
	{
		Turn(robot,pp,LEFTY);
		MoveLeft(robot,pp,0.5);
		cout << "*****GO LEFT" <<endl;
	}
	else if ((lp[45] < 1) && (lp[87] < 1) && (lp[0] > 1) && (Yaw >0.75) && (Yaw < 1.90)) //facing up, no left, turn right
	{
		Turn(robot,pp,RIGHTY);
		MoveRight(robot,pp,0.5);
		cout << "******GO RIGHT" <<endl;
	}

	else if ((lp[45] > 3) && (lp[87] < 1) && (Yaw > -0.5) && (Yaw < 0.5)){ //facing right, no left. go straight
		MoveRight(robot,pp,1);
	}
	else if ((lp[45] > 3) && (lp[87] > 1) && (Yaw > -0.5) && (Yaw < 0.5) ){ //facing right, turn left room
		MoveRight(robot,pp,0.5);
		Turn(robot,pp,UPY);
		cout << "******GO LEFT" <<endl;
		MoveUp(robot,pp,1);
	}
	else if ((lp[45] > 3) && (lp[87] > 1) && (Yaw > -0.5) && (Yaw < 0.5) ){ //facing right, turn left
		MoveRight(robot,pp,0.5);
		Turn(robot,pp,UPY);
		cout << "******GO LEFT" <<endl;
		MoveUp(robot,pp,1);
	}
	else if ((lp[45] < 3) && (lp[87] < 1) && (Yaw > -0.5) && (Yaw < 0.5) ){ //facing right, no left, go right
		MoveDown(robot,pp,0.5);
		Turn(robot,pp,DOWNY);
		cout << "******GO RIGHT" <<endl;
		MoveDown(robot,pp,1);
	}
	else if ((lp[45] > 3) && (lp[87] < 1) && (Yaw >2.7) || (Yaw < -1.75) ){ //facing left, no left, go straight
		MoveLeft(robot,pp,1);
	}
	else if ((lp[45] < 2) && (lp[87] > 1) && (Yaw >2.7) || (Yaw < -1.75) ){ //facing left, turn  left
		MoveLeft(robot,pp,0.5);
		Turn(robot,pp,DOWNY);
		MoveDown(robot,pp,1);
	}
	else if ((lp[45] < 2) && (lp[87] < 1) && (Yaw >2.7) || (Yaw < -1.75) ){ //facing left, turn Right 

		Turn(robot,pp,UPY);
		MoveUp(robot,pp,0.5);
	}
	else if ((lp[45] > 2) && (lp[87] > 1) && (Yaw >2.7) || (Yaw < -1.75) ){ //facing left, turn left room
		Turn(robot,pp,DOWNY);
		MoveDown(robot,pp,1);
	}

	else if ((lp[45] > 1) && (lp[87] < 1) && (Yaw > -1.9) && (Yaw < -0.3) ){ //facing down, no left, go straight
		MoveDown(robot,pp,1);
	}
	else if ((lp[45] < 1) && (lp[87] > 1) && (Yaw >2.7) && (Yaw < -1.75) ){ //facing down, turn  left
		MoveDown(robot,pp,0.5);
		Turn(robot,pp,RIGHTY);
		MoveRight(robot,pp,1);
	}
	else if ((lp[45] < 1) && (lp[87] < 1) && (Yaw >2.7) && (Yaw < -1.75) ){ //facing down, turn Right 
		MoveDown(robot,pp,0.5);
		Turn(robot,pp,LEFTY);
		MoveLeft(robot,pp,0.5);
	}
	else if ((lp[45] > 1) && (lp[87] > 1) && (Yaw >2.7) && (Yaw < -1.75) ){ //facing Down, turn left room
		Turn(robot,pp,LEFTY);
		MoveLeft(robot,pp,1);
	}
...
cout <<"FINISHED HAHA HA" << endl;
/*

	while(true) {
		//look all directions and take in data to GRID
		Turn(robot,pp,DOWNY);
		Fill_Laser_Data(robot,pp,lp,GRID,row,col);
		Turn(robot,pp,LEFTY);
		Fill_Laser_Data(robot,pp,lp,GRID,row,col);
		Turn(robot,pp,UPY);
		Fill_Laser_Data(robot,pp,lp,GRID,row,col);
...
*/
	...
	value_iteration(GRID,NAV,2,15,row,col);
